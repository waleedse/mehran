...
class Discount_codes extends Component {
    constructor(props) {
        super(props);
        this.state={
            name:'',
    ...
    componentDidMount(){
        ...
        Axios.post('/api/get_discount_codes').then(res=>{
            this.setState({
                discounts:res.data
            })
        })
    ...
    enddate(e){
        this.setState({
            ...
            console.log(this.state.enddate.getMonth())
        })
    }
    create_discount(e){
        let senderdata = {
            ...
            startdate:this.state.startdate.getFullYear()+'/'+(parseInt(this.state.startdate.getMonth())+1)+1+'/'+this.state.startdate.getDate(),
            enddate:this.state.enddate.getFullYear()+'/'+(parseInt(this.state.enddate.getMonth())+1)+1+'/'+this.state.enddate.getDate(),

        }
        console.log(senderdata);
        Axios.post('/api/create_discount_codes',senderdata).then(res=>{
    ...
    update_discount(e){
        ...
        console.log(senderdata);
        Axios.post('/api/update_discount_codes',senderdata).then(res=>{
            this.componentDidMount();
            this.setState({
                edit_modal:false
            })
    ...
    render() {
        return (
            <div>
                <div className="p-3">
                    <button style={{float:'right'}} onClick={this.show_modal.bind(this)} className="btn btn-success">Create Discount Code</button>
                </div>
                <div className="p-4">
