...
class AddDiscountProducts extends Component {
    constructor(props) {
        super(props);
        this.state={
            products:[],
    ...
    get_varients_by_product_id(){
        let senderdata = {
            distributor_id:this.props.id,
            product_id:this.state.product_id
        }
        Axios.post('/api/get_distributor_product_variations',senderdata).then(res=>{
            this.setState({
                varients:res.data
            })
        })
    ...
    update_varient(id,discount){
        ...
        Axios.post('/api/update_distributor_varients',senderdata).then(res=>{
            Swal.fire({
                icon: 'success',
...
export default AddDiscountProducts;