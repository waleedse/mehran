...
class Orderslist extends Component {
    ...
    componentDidMount(){
        Axios.post(baseurl+'/api/get_dis_all_orders').then(res=>{
            this.setState({
                orders:res.data
            })
        })
    ...
    search_records(){
        ...
        Axios.post('/api/search_dis_orders',senderdata).then(res=>{
            this.setState({
                orders:res.data
            })
        })
        ...
        return (
            <div className="container">
                <div className="top_section_title_div">
                    <h2 className="section_title">Distributor Order Lists</h2>
                </div>
                        ...
                        <div class="form-group input_div col-md-12">
                            <div className="row mb-2">
                                <div className="col-md-10">
                                    ...
                                    aria-describedby="emailHelp" placeholder="Search by Order Id, Distributor Id, Name, Email, Phone, Date" />
                                </div>
                            ...
                            this.state.orders.map((data,index)=>{
                                return(
                                    <tr>
                                        ...
                                        <td>{data.totals}</td>
                                        <td>{data.status}</td>
                                        <td><Link to={`/adminpanel/distributor-order-details/${data.id}`}><button className="btn btn-outline-success"> <i  className="fas fa-eye"> </i></button></Link></td>
                                    </tr>
                                )
                            })
                        }
