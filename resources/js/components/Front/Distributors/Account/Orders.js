...
class MyOrders extends Component {
    ...
    componentDidMount(){
        setTimeout(()=>{
            let senderdata = {
                dis_id:this.props.user.dis
            }
            console.log(senderdata);
            Axios.post(baseurl+'/api/get_dis_orders_by_customer_id',senderdata).then(res=>{
                console.log(res);
                this.setState({
    ...
    show_order_details(id){
        ...
        window.localStorage.setItem('d_oid',id);
        this.props.history.push('/distributor/order-details');
    }
    render() {
        return (
            <div>
